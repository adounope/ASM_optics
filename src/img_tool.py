...
def arr_2_mp4(arr, path, fps = 30, title=lambda t: str(t), x_label='', y_label=''):
    ...
    def update(frame):
        ...
        return heatmap,
    ani = FuncAnimation(fig, update, frames=arr.shape[0], blit=True, interval=100)
...
#     g = lambda x: f(x.reshape(kx, ky))
#     for i in range(out_dim[0]):
#         for j in range(out_dim[1]):
#             conv_out[..., i, j] = np.apply_along_axis(g, axis=-1, arr=arr[..., i:i+kx, j:j+ky].reshape(*shape[:-2], -1))
#     return conv_out

def conv_lambda2D(arr, kernel_shape = (3,3), f = lambda x: x.mean(axis=(-1,-2))):
    '''
    allow a lambda to take array within 2D window as input, slide window over entire last 2 dim of ND array
    arr: <ndarray> *, Nx, Ny
    f: <func> must explicitely operate on last 2 dimensions only
    '''
    shape = arr.shape # *, Nx, Ny
    kx, ky = kernel_shape
    out_dim = (shape[-2] - kx + 1, shape[-1] - ky + 1)
